...
const handleRegistration = async (req, res, next) => {
  try {
    ...
    topic.topic_group_student[studentIndex].status = action === 'approve' ? 'approved' : 'rejected';
    ...
    await User.findByIdAndUpdate(studentId, {
      $set: {
        'registered_topics.$[elem].status': action === 'approve' ? 'approved' : 'rejected',
    ...
    await Notification.create({
      ...
      user_notification_content: feedback || `Giảng viên đã ${action === 'approve' ? 'chấp nhận' : 'từ chối'} đăng ký của bạn.`,
...
const getGuidedStudents = async (req, res, next) => {
  try {
    ...
    const topics = await Topic.find({
      ...
      .populate('topic_group_student.student', 'user_name user_id email user_phone user_major user_average_grade')
    ...
    const stats = {
      ...
      topics: topics.map(topic => ({
        topic_id: topic._id,
        topic_title: topic.topic_title,
        student_count: topic.topic_group_student.filter(m => m.status === 'approved').length,
